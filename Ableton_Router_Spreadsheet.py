...
def load_spreadsheet_data_csv():
    try:
        ...
        st.error("Error: Timed out while fetching the spreadsheet.")
    ...
    for channel in sorted(all_channels):
        ...
        if "/" in channel:
            ...
            try:
                ...
                st.warning(f"Invalid channel format '{channel}'. Skipping.")
    return channel_map

def map_channel_to_target(channel, channel_map):
    if channel not in channel_map:
        st.warning(f"Unknown channel '{channel}'. Skipping track.")
        return None
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for track in root.findall(".//AudioTrack") + root.findall(".//MidiTrack") + root.findall(".//GroupTrack"):
                ...
                ET.SubElement(output_elem, "Target").set("Value", routing_dict["Target"])
                ET.SubElement(output_elem, "UpperDisplayString").set("Value", "Ext. Out")
                ET.SubElement(output_elem, "LowerDisplayString").set("Value", routing_dict["LowerDisplayString"])
                ET.SubElement(output_elem, "MpeSettings").text = None
                ...
                for manual in speaker.findall("Manual"):
                    speaker.remove(manual)
                ET.SubElement(speaker, "Manual").set("Value", "false" if mute else "true")

                if db_reduction is not None and db_reduction != 0.0:
                    volume = mixer.find("Volume") or ET.SubElement(mixer, "Volume")

                    # FIX: Remove any duplicate <Manual> elements before adding one
                    for manual in volume.findall("Manual"):
                        volume.remove(manual)

                    manual_volume = ET.SubElement(volume, "Manual")
                    manual_volume.set("Value", "0.794328")  # Default fallback
            ...
            with open(temp_modified_xml, "rb") as f_in:
                output_bytes = gzip.compress(f_in.read())
...
def main():
    ...
    if not CAMPUSES:
        st.error("No campuses found in the spreadsheet.")
        return

    channel_map = generate_channel_map(df, campus_columns)
    selected_campus = st.selectbox("Select Campus", CAMPUSES)
    uploaded_files = st.file_uploader(f"Upload .als files for {selected_campus}", type=["als"], accept_multiple_files=True)
    ...
    if uploaded_files:
        for uploaded_file in uploaded_files:
            ...
            if not original_filename.lower().endswith(".als"):
                st.warning(f"Skipping {original_filename}: not a .als file.")
                continue
            ...
            if output_bytes:
                st.success(f"Processed {output_filename}")
                st.download_button(
