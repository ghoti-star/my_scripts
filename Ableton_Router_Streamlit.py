            ...
            for track_type in ["AudioTrack", "MidiTrack", "GroupTrack"]:
                for track in root.findall(f".//{track_type}"):
                    ...
                    if not track_name or track_name.strip() == "":
                        st.write(f"Skipping track with no name or empty name, XML Path: {track.tag}")
                        continue

                    # --- Routing Logic ---
                    device_chain = track.find("DeviceChain")
                    if device_chain is None:
                        st.write(f"No DeviceChain found for {track_name}, creating one, XML Path: {track.tag}")
                        device_chain = ET.SubElement(track, "DeviceChain")

                    output_elem = device_chain.find("AudioOutputRouting")
                    if output_elem is None:
                        st.write(f"No AudioOutputRouting found for {track_name}, creating one, XML Path: {track.tag}")
                    ...
                    st.write(f"Track: {track_name}, Current Routing: {current_routing}, XML Path: {track.tag}")
                    ...
                    for keyword, routing_dict in ROUTING_MAP.items():
                        if keyword.upper() == track_name.upper():
                            ...
                            st.write(f"Updated Routing: {track_name} → {routing_dict['Target']} ({routing_dict['LowerDisplayString']})")
                            st.write(f"Final AudioOutputRouting for {track_name}:")
                            st.write(ET.tostring(output_elem, encoding='unicode').strip())
                            st.write(f"Parent DeviceChain for {track_name}:")
                            st.write(ET.tostring(device_chain, encoding='unicode').strip())
                            found_match = True
                            break

                    if not found_match:
                        st.write(f"No matching routing for {track_name}—keeping current routing: {current_routing}")

                    # --- Mute Logic (Updated to use Speaker) ---
                    mixer = device_chain.find("Mixer")
                    if mixer is None:
                        st.write(f"No Mixer found for {track_name}, creating one, XML Path: {track.tag}")
                        mixer = ET.SubElement(device_chain, "Mixer")

                    speaker = mixer.find("Speaker")
                    if speaker is None:
                        st.write(f"No Speaker found for {track_name}, creating one, XML Path: {track.tag}")
                    ...
                    if track_name.upper() in MUTE_TRACKS:
                        ...
                        st.write(f"Muted track: {track_name} (Speaker was {current_speaker_state})")

                    # --- Volume Adjustment Logic ---
                    volume = mixer.find("Volume")
                    if volume is None:
                        st.write(f"No Volume found for {track_name}, creating one, XML Path: {track.tag}")
                    ...
                    if track_name.upper() in TURN_DOWN_TRACKS:
                        ...
                        st.write(f"Adjusted volume for {track_name}: {current_volume} ({current_db:.2f} dB) → {new_volume} ({new_db:.2f} dB)")
